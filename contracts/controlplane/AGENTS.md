...
传输边界。当前范围是 `/v1` 版本前缀、Daemon/Project/Change/Ticket Graph 强类型 DTO、结构化错误
...
- Ticket Graph ReadModel 只暴露稳定 Graph/Ticket/Dependency/Frontier 结构；不得暴露 GenerationKey、内部 commit identity、Ticket execution state、Lease、Assignment 或授权字段。
