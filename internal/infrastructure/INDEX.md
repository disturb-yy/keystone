...
`config`、`logging`、`id` 之间没有代码依赖；`localstate` 只依赖操作系统文件能力和平台锁原语，`migration` 只依赖 `database/sql` 与纯 Go SQLite driver。各 package 都不访问 Domain、业务表或其他进程。
