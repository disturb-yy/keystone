...
该 package 已提供真实的 `keystone init`、`keystone change ...`（包括只读
`change ticket-graph CHANGE_ID`）和
...
| `http_client.go` | Control Plane health/status/stop 和 Ticket Graph 的 JSON client |
| `change_command.go` | Change create/list/show、Ticket Graph 查询、生命周期控制和 HumanDecision CLI 命令 |
...
只读取 metadata 并发起 HTTP 请求，Change 读取命令（包括 `ticket-graph`）不启动新 Daemon、
不触发 Ticketize 或写入数据库。
