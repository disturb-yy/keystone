...
`keystone daemon start` 是唯一直接实现实例 ensure 的命令，必须在有界等待内确认目标实例已健康后才成功返回；`keystone init` 只能委托同一 Application seam，不能另行拥有启动、锁或 SQLite 生命周期。`status` 和 `stop` 从不隐式启动实例。metadata 缺失、endpoint 不可达或 instance ID 不匹配均为明确失败且不触碰现有本机状态；同一实例已接受的重复 stop 可再次返回已接受，不要求 `Idempotency-Key`。
